...
# @Last Modified time: 2022-10-19 16:15:40
...
class WGBase:
    ...
    def get_wireguard_base_info(self, event):
        if self.tunnel_charm.model.unit.is_leader():
            ...
            try:
                event.set_results({'output':  json.dumps(result), "errors": ""})
            except:
                logging.info("Results were not set.")
    ...
    def update_wg_ip(self, event):
        if self.tunnel_charm.model.unit.is_leader():
            ...
            m_wgconfig.add_attr(None, 'Address', new_ip + "/24")
            m_wgconfig.write_file()
